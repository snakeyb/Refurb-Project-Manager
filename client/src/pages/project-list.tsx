...
import { Plus, Search, Building2, ExternalLink, Hammer, Copy, LayoutTemplate } from "lucide-react";
...
type ViewTab = "Projects" | "Templates";
...
export default function ProjectList() {
  ...
  const filtered = projects?.filter((p) => {
    const matchesTab = activeTab === "Templates" ? p.isTemplate : !p.isTemplate;
  ...
  const noun = activeTab === "Templates" ? "template" : "project";
  const nounPlural = activeTab === "Templates" ? "templates" : "projects";
  ...
  return (
    <div className="min-h-screen bg-background" data-testid="page-project-list">
      <EspoHeader
        ...
        actions={
          activeTab === "Templates" ? (
            <Link href="/projects/new?template=1">
      ...
      <div className="p-3 sm:p-5">
        <div className="mb-4 flex items-center gap-3 flex-wrap">
          <Tabs value={activeTab} onValueChange={(v) => { setActiveTab(v as ViewTab); setSearch(""); }}>
            <TabsList>
          ...
          <span className="text-xs text-muted-foreground" data-testid="text-project-count">
            {filtered?.length ?? 0} {(filtered?.length ?? 0) !== 1 ? nounPlural : noun}
          </span>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="border rounded-md p-8 bg-card text-center" data-testid="error-state">
            <p className="text-muted-foreground">Failed to load refurb {nounPlural}. Please try again.</p>
          </div>
                ...
                <thead>
                  <tr className="border-b bg-white dark:bg-card">
                    <th className="text-left py-2.5 px-4 font-medium text-muted-foreground text-xs">
                      {activeTab === "Templates" ? "Template Name" : "Project Name"}
                    </th>
                    {activeTab === "Projects" && (
                      ...
                      <td className="py-3 px-4">
                        <Link href={`/projects/${project.id}`} className="block">
                          <div className="flex items-center gap-2">
                            {project.isTemplate ? (
                              <LayoutTemplate className="h-3.5 w-3.5 text-primary flex-shrink-0" />
                            ) : (
                      ...
                      <td className="py-3 px-2">
                        <div className="flex items-center justify-end gap-1">
                          <Button
                            ...
                            title={project.isTemplate ? "Use template" : "Duplicate project"}
                  ...
                  <div className="flex items-start justify-between gap-2 mb-1.5">
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      ...
                      <Button
                        ...
                        title={project.isTemplate ? "Use template" : "Duplicate"}
          ...
          <div className="border rounded-md p-12 bg-card text-center" data-testid="empty-state">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              {search
                ...
                : activeTab === "Templates"
                  ? "Create a template to reuse standard line items across multiple projects."
                  : "Get started by creating your first refurbishment project."}
            </p>
            {!search && (
              <Link href={activeTab === "Templates" ? "/projects/new?template=1" : "/projects/new"}>
                <Button size="sm" data-testid="button-create-first">
                  <Plus className="h-3.5 w-3.5 mr-1.5" />
                  {activeTab === "Templates" ? "Create Template" : "Create Refurb Project"}
                </Button>
              </Link>
            )}
          </div>
